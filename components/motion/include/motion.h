...
typedef enum { ELS_STATE_IDLE=0, ELS_STATE_WAITING, ELS_STATE_RUNNING, ELS_STATE_RETRACTING, ELS_STATE_PAUSED, ELS_STATE_ERROR } els_state_t;
typedef struct { float pitch_mm; stepper_dir_t feed_dir; float z_start_mm, z_end_mm; uint8_t passes; float depth_per_pass; bool left_hand; } els_config_t;
typedef struct { els_state_t state; float pitch_mm; uint8_t pass_current, pass_total; float z_pos_mm; uint16_t spindle_rpm; int32_t steps_sent; bool sync_ok; bool left_hand; } els_status_t;
