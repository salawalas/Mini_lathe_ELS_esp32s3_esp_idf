...
// ── RGB565 → RGB666 lookup tables (ILI9488 18-bit) ──
static uint8_t _r5_to_byte[32];
static uint8_t _g6_to_byte[64];
static uint8_t _b5_to_byte[32];
static bool _lut_init = false;

static void _init_lut(void)
{
	if (_lut_init) return;
	for (int i = 0; i < 32; i++) {
		uint8_t v = (i << 3) | (i >> 2);
...
{
	if (dev->_model == 0x9488) {
		// ILI9488 18-bit: batch RGB565→RGB666, max 341 px/batch
		_init_lut();
		uint8_t r = _r5_to_byte[(color >> 11) & 0x1F];
		uint8_t g = _g6_to_byte[(color >> 5) & 0x3F];
		uint8_t b = _b5_to_byte[color & 0x1F];
		uint8_t buf[341 * 3];
		int max_batch = 341;
		for (int j = 0; j < max_batch; j++) {
			buf[j*3+0] = r; buf[j*3+1] = g; buf[j*3+2] = b;
		}
		uint16_t rem = size;
		gpio_set_level(dev->_dc, SPI_Data_Mode);
		while (rem > 0) {
			uint16_t n = rem > max_batch ? max_batch : rem;
			spi_master_write_byte(dev->_TFT_Handle, buf, n * 3);
			rem -= n;
		}
		return true;
	}
...
{
	if (dev->_model == 0x9488) {
		// ILI9488 18-bit: convert RGB565→RGB666, batch max 341 px
		_init_lut();
		uint8_t buf[341 * 3];
		uint16_t rem = size;
		uint16_t *src = colors;
		gpio_set_level(dev->_dc, SPI_Data_Mode);
		while (rem > 0) {
			uint16_t n = rem > 341 ? 341 : rem;
			for (uint16_t i = 0; i < n; i++) {
				uint16_t c = src[i];
				buf[i*3+0] = _r5_to_byte[(c >> 11) & 0x1F];
				buf[i*3+1] = _g6_to_byte[(c >> 5) & 0x3F];
				buf[i*3+2] = _b5_to_byte[c & 0x1F];
			}
			spi_master_write_byte(dev->_TFT_Handle, buf, n * 3);
			src += n;
			rem -= n;
...
void lcdInit(TFT_t *dev, uint16_t model, int width, int height, int offsetx, int offsety)
{
	...
	if (dev->_model == 0x9340 || dev->_model == 0x9341 || dev->_model == 0x7735)
	{
		...
		spi_master_write_comm_byte(dev, 0x36); // Memory Access Control
		// Set MADCTL for ST7735 orientation only (keep color order unchanged)
		// Changed from 0x08 (default) to 0x60 to match this TFT's needed orientation
		spi_master_write_data_byte(dev, 0x60); // Memory Access Control value (MADCTL)
	...
	if (dev->_model == 0x9488)
	{
		...
		// Hardware reset już wykonany w spi_master_init.
		// Potrzebny odstęp ≥120ms przed pierwszą komendą SPI.
		delayMS(150);

		// Positive Gamma Control (TFT_eSPI)
		...
		// Negative Gamma Control (TFT_eSPI)
		...
		spi_master_write_data_byte(dev, 0x46);
		...
		// Power Control 1 (TFT_eSPI)
		spi_master_write_comm_byte(dev, 0xC0);
		spi_master_write_data_byte(dev, 0x17);
		spi_master_write_data_byte(dev, 0x15);

		// Power Control 2
		spi_master_write_comm_byte(dev, 0xC1);
		spi_master_write_data_byte(dev, 0x41);

		// VCOM Control (TFT_eSPI)
		spi_master_write_comm_byte(dev, 0xC5);
		spi_master_write_data_byte(dev, 0x00);
		spi_master_write_data_byte(dev, 0x12);
		spi_master_write_data_byte(dev, 0x80);

		// Memory Access Control: MV=1, BGR=1 → landscape 480x320
		spi_master_write_comm_byte(dev, 0x36);
		spi_master_write_data_byte(dev, 0x28);

		// Pixel Format – 18-bit (TFT_eSPI standard for SPI)
		spi_master_write_comm_byte(dev, 0x3A);
		spi_master_write_data_byte(dev, 0x66);

		// Interface Mode Control
		spi_master_write_comm_byte(dev, 0xB0);
		spi_master_write_data_byte(dev, 0x00);

		// Frame Rate Control
		spi_master_write_comm_byte(dev, 0xB1);
		spi_master_write_data_byte(dev, 0xA0);

		// Display Inversion Control
		spi_master_write_comm_byte(dev, 0xB4);
		spi_master_write_data_byte(dev, 0x02);

		// Display Function Control
		spi_master_write_comm_byte(dev, 0xB6);
		spi_master_write_data_byte(dev, 0x02);
		spi_master_write_data_byte(dev, 0x02);
		spi_master_write_data_byte(dev, 0x3B);

		// Entry Mode Set
		spi_master_write_comm_byte(dev, 0xB7);
		spi_master_write_data_byte(dev, 0xC6);
		...
		// Sleep Out
		spi_master_write_comm_byte(dev, 0x11);
		delayMS(120);
		...
		// TEST: wypełnij czerwonym – 18-bit (użyj spi_master_write_color)
		spi_master_write_comm_byte(dev, 0x2C);
		spi_master_write_color(dev, 0xF800, (uint16_t)width * height);
	} // endif 0x9488
...
{
	if (dev->_model == 0x9340 || dev->_model == 0x9341 || dev->_model == 0x7735 || dev->_model == 0x7789 || dev->_model == 0x7796 || dev->_model == 0x9488)
	{
